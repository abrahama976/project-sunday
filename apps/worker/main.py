...
async def handle_message(client: Client, message: dict, history: list):
...
async def poll_approved(client: Client):
    while True:
        ...
        try:
            rows = client.table("action_queue").select("*").eq("approved", True).eq("status", "pending").execute()
        ...
        try:
            ...
            if all_msgs.data and all_msgs.data[0]["role"] == "user":
                ...
                if latest["id"] != last_processed_id:
                    ...
                    await handle_message(client, latest, history_list)
